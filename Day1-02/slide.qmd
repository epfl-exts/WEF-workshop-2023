...
flowchart BT
    ...
    linkStyle 1,2 stroke:#bcf5bc,stroke-width:6px
%%    style A fill:#bcf5bc
    style B fill:#bcf5bc
%%    style C fill:#bcf5bc
%%    style D fill:#bcf5bc
...
::: notes
in this session we review some relevant topic in statistics which is going to be the basis for the bayesan statistics as well as AB testin we will see in the next sessions
:::
...
::: notes
Distributions describe how values are spread over a certain field. for instance, most of you are sitting in this part of the room and few or no one in the other areas or corners.
...
values can come from natural phenomena, e.g. age of people in a country
values can come from an experiment, e.g. number of heads in a coin toss
values can be discrete e.g. number of heads in a coin toss
values can be continuous e.g. waiting time for a bus
values can be available to us or not e.g. we can measure the height of people in a room, but we can not measure the height of all people in the world

as you see Distributions are all around us, so it makes sense to formulate them in a general way. That's where we have statistical distributions: e.g. Normal, Beta, Gamma, . . .
:::
...
<!-- ![](img/normal.png) -->
$\frac{1}{\color{red}\sigma \sqrt{2\pi} } e^{-\frac{1}{2}\left(\frac{\color{blue}x-\color{red}\mu}{\color{red}\sigma}\right)^2}$
:::
...
<!-- ![](img/beta.png) -->
$\frac{1}{B(\color{red}\alpha,\color{red}\beta)} \color{blue}x^{\color{red}\alpha-1}(1-\color{blue}x)^{\color{red}\beta-1}$
:::
...
<!-- ![](img/gamma.png) -->
$\frac{ \color{blue}x^{\color{red}\alpha-1} e^{-\color{red}\beta \color{blue}x} \color{red}\beta^\color{red}\alpha}{\Gamma(\color{red}\alpha)}$
:::
...
::: notes
Distributions are all around us, so it makes sense to formulate them in a general way. That's where we have statistical distributions: e.g. Normal, Beta, Gamma, . . .
What I want to note here is also that range of values can be different for different distributions. For instance, Normal distribution is defined over all real numbers, while Beta distribution is defined over the range [0,1].

but what about the shape of these distributions? what defines them? what are their properties? we will see in the next slides.
:::
...
::: notes
mean: how frequent values are in a certain area
variance: how values are spreaded around the mean
skewness: how frequent values are in a certain area in an asymmetric way
...
<div style="position:relative; width:640px; height:480px; margin:0 auto;">
  ...
  <img class="fragment current-visible" data-fragment-index="1" src="img/pmf.png" style="position:absolute;top:0;left:0;" />
  <img class="fragment current-visible" data-fragment-index="2" src="img/cdf.png"     style="position:absolute;top:0;left:0;" />
  </div>
::: 
:::
<!---
...
> Learning from a sample versus learning from full distribution❓ 

<br>

Can we learn from the full distribution? 
...
- [Please be back here at 12:00]{style="color:#ff412c; font-weight: bold;"}
