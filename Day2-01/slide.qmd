...
:::
::: {.column width="40%" }
![](img/AdobeStock_519277519.jpeg){width="75%" fig-align="left"}
:::
:::
...
::: 
::: {.column width="40%" }
![](img/AdobeStock_120197715.jpeg){width="75%" height="75%" fig-align="left"}
:::
:::
...
::: columns
::: {.column width="60%" }
This is where we need PyMC 🌱, a Python library for Bayesian statistical modeling, and we use it to do the Bayesian inference for the AB testing problem.
...
<br>
Our goal is to find out `revenue per visitor`. The expected revenue per visitor is $\frac{\theta}{\lambda}$. So we need:
...
> We should go the same way as before:\ we need to define the priors, likelihoods and posteriors for $\theta$ and $\lambda$.
...
We obtain the `revenue per visitor` for each variants by:
