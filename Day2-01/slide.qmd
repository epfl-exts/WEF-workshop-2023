...
-  For simplicity, they follow the same distribution: $\alpha_A=\beta_A=\alpha_B=\beta_B$
...
- Posterior is also Beta distributed but with different parameters
- If no conjugate priors are used, we can still sample from the posterior
...
$$\lambda: \left\{
    \begin{array}{ll}
    \lambda_A \sim \mathrm{Gamma}(\alpha_2 + rev_A, \beta_2 + conv_A) \\
    \lambda_B \sim \mathrm{Gamma}(\alpha_2 + rev_B, \beta_2 + conv_B) 
    \end{array}
\right.$$
